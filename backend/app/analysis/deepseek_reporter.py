...
class DeepSeekHotReporter:
    ...
    def analyze(self, topic: str, candidates: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        try:
            response = self.client.post(f"{self.base_url}/chat/completions", headers={"Authorization": f"Bearer {self.api_key}"}, json={"model": self.model, "response_format": {"type": "json_object"}, "messages": [{"role": "system", "content": "Return JSON only. Analyze only supplied public evidence; disclose unknown publication dates."}, {"role": "user", "content": json.dumps(prompt, ensure_ascii=False)}]})
